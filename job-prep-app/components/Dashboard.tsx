...
export default function Dashboard({ spec, applications, onSpecChange, onApplicationsChange, onReset }: Props) {
  ...
  const [historyView, setHistoryView] = useState<{
    featureKey: DocFeature | "organize";
    title: string;
    endpoint: string;
  ...
  function getHistoryKey(featureKey: string, appId: string | null) {
    return `job-prep-history:${featureKey}:${appId ?? "no-app"}`;
  }

  function loadHistory(featureKey: string, appId: string | null): HistoryItem[] {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {[
                  (Array.isArray(goals.targetRole) ? goals.targetRole : [goals.targetRole]).filter(Boolean).join(", ") || "직무 미설정",
                  (Array.isArray(goals.targetIndustry) ? goals.targetIndustry : [goals.targetIndustry]).filter(Boolean).join(", ") || "업종 미설정",
                ].join(" · ")} 준비 중
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <SpecCard title="목표" icon="🎯">
              <InfoRow label="직무" value={(Array.isArray(goals.targetRole) ? goals.targetRole : [goals.targetRole]).filter(Boolean).join(", ") || "미설정"} />
              <InfoRow label="업종" value={(Array.isArray(goals.targetIndustry) ? goals.targetIndustry : [goals.targetIndustry]).filter(Boolean).join(", ") || "미설정"} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="font-bold text-gray-800 text-base">{historyView.title}</h2>
