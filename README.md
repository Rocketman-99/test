...
앱을 열 때 마지막 백업이 24시간 넘게 지났으면, 로컬 서버가 프로젝트 폴더의
`backups/`에 자동으로 한 부 저장한다. 별도 조작이 필요 없다.

- 최근 **10개**만 보관하고 오래된 것은 자동 정리된다
- **자동 백업에는 API 키가 들어가지 않는다.** 파일이 디스크에 계속 쌓이기 때문
- `backups/`는 `.gitignore` 대상이라 저장소에 올라가지 않는다
- **💾 데이터 백업** 섹션의 `백업 목록`에서 시점을 골라 되돌릴 수 있다
- `지금 백업`으로 즉시 한 부 남길 수도 있다

같은 PC에 저장되므로 **디스크 고장이나 폴더 삭제까지 대비하려면 `내보내기`로 받은
파일을 별도 위치에 보관**해야 한다.

### 파일로 내보내기 / 불러오기

다른 브라우저나 PC로 옮길 때 쓴다.

- API 키는 기본적으로 포함되지 않는다 (체크박스로 포함 선택 가능)
